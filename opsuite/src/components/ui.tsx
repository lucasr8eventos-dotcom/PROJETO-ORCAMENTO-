  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ width: 36, height: 36, borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', background: iconBg }}>
          {icon}
        </div>
        {badge && (
          <span style={{
...
// ── Empty state ───────────────────────────────────────────────────────────────
export function Empty({ message = 'Nenhum item encontrado' }: { message?: string }) {
  return (
...
export function Modal({ open, onClose, title, children, width = 520 }: {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: 'var(--surface)', borderRadius: 16, width: '100%', maxWidth: width,
    ...
    <div style={{ display: 'flex', gap: 2, background: 'var(--surface2)', borderRadius: 10, padding: 3, border: '1px solid var(--border)', overflowX: 'auto' }}>
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)} style={{
          padding: '7px 16px', borderRadius: 7, fontSize: 13, fontWeight: active === t.id ? 500 : 400,
...
export function Avatar({ name, size = 38 }: { name: string; size?: number }) {
  ...
  return (
    <div style={{
      ...
      fontSize: size * 0.33, fontWeight: 600, color: '#fff', flexShrink: 0,
    }}>{initials}</div>
  );
}
